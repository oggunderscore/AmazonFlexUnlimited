...
class FlexUnlimited:
    ...
    def __init__(self) -> None:
        ...
        if (
            twilioAcctSid != ""
            and twilioAuthToken != ""
            and self.twilioFromNumber != ""
            and self.twilioToNumber != ""
        ):
            ...
            for day in desiredWeekdays:
                ...
                if dayAbbreviated not in weekdayMap:
                    print(
                        "Weekday '"
                        + day
                        + "' is misspelled. Please correct config.json file and restart program."
                    )
                    exit()
    ...
    def __registerAccount(self):
        ...
        res = self.session.post(
            FlexUnlimited.routes.get("GetAuthToken"),
    ...
    def __generate_frc(device_id):
        ...
        hmac_ = hmac.new(
            PBKDF2(device_id, b"HmacSHA256").read(32), iv + ciphertext, hashlib.sha256
        ).digest()
    ...
    def __getFlexAccessToken(self):
        ...
        res = self.session.post(
            FlexUnlimited.routes.get("RequestNewAccessToken"),
            json=data,
            headers=headers,
        ).json()
    ...
    def run(self):
        ...
        while self.__retryCount < self.retryLimit:
            ...
            if offersResponse.status_code == 200:
                ...
                currentOffers.sort(
                    key=lambda pay: int(pay["rateInfo"]["priceAmount"]), reverse=True
                )
                # print(currentOffers)
